...
namespace TerraSdk.Test.Crypto.Bip32
{
    ...
    public class Ed25519HdKeyTests
    {
        ...
        public void TestValidKey()
        {
            ...
            var key = Ed25519HdKey.GetMasterKeyFromSeed(seed.ToByteArrayFromHex());

            Assert.Equal(expectedKey, key.Key.ToHexFromByteArray());
            Assert.Equal(expectedChainCode, key.ChainCode.ToHexFromByteArray());
        }
        ...
        {
            var derivedKey = Ed25519HdKey.DerivePath(path, seed.ToByteArrayFromHex());


            Assert.Equal(chainCode, derivedKey.ChainCode.ToHexFromByteArray());
            Assert.Equal(key, derivedKey.Key.ToHexFromByteArray());
            Assert.Equal(publicKey, Ed25519HdKey.GetPublicKey(derivedKey.Key).ToHexFromByteArray());
        }

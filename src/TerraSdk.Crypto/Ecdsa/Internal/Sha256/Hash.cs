...
namespace TerraSdk.Crypto.Ecdsa.Internal.Sha256
{
    internal class Hash
    {
        ...
        public static void Write(Sha256T hash, UInt32[] data, UInt32 len)
        {
            ...
            Secp256K1.Util.Memcpy(data, 0, byfdata, 0, byfdata.Length);
        ...
        public static void Write(Sha256T hash, byte[] data, UInt32 len)
        {
            ...
            while (bufsize + len >= 64)
            {
                ...
                Secp256K1.Util.Memcpy(data, index, hash.Buf, bufsize, 64 - bufsize);//memcpy(((unsigned char*)hash->buf) + bufsize, data, 64 - bufsize);
            ...
            if (len > 0)
            {
                ...
                Secp256K1.Util.Memcpy(data, index, hash.Buf, bufsize, len);//memcpy(((byte[])hash->buf) + bufsize, data, len);
            }
        }
...
#endif
            Secp256K1.Util.Memcpy(byfOut, 0, out32, 0, 32);//memcpy(out32, (const byte[])byfOut, 32);
        }
        ...
        private static void HmacSha256Initialize(HmacSha256T hash, byte[] key, UInt32 keylen)
        {
            ...
            if (keylen <= 64)
            {
                Secp256K1.Util.Memcpy(key, 0, rkey, 0, keylen);//memcpy(rkey, key, keylen);
                Secp256K1.Util.MemSet(rkey, keylen, 0, 64 - keylen);//memset(rkey + keylen, 0, 64 - keylen);
            }
            else
            {
                ...
                Secp256K1.Util.MemSet(rkey, 32, 0, 32); //memset(rkey + 32, 0, 32);
            }
            ...
            Secp256K1.Util.MemSet(rkey, 0, 64);//memset(rkey, 0, 64);
        }
        ...
        static void HmacSha256Finalize(HmacSha256T hash, byte[] out32)
        {
            ...
            Secp256K1.Util.MemSet(temp, 0, 32);//memset(temp, 0, 32);
        ...
        public static void Rfc6979HmacSha256Initialize(Rfc6979HmacSha256T rng, byte[] key, UInt32 keylen)
        {
            ...
            Secp256K1.Util.MemSet(rng.V, 0x01, 32);//memset(rng->v, 0x01, 32); /* RFC6979 3.2.b. */
            Secp256K1.Util.MemSet(rng.K, 0x00, 32);//memset(rng->k, 0x00, 32); /* RFC6979 3.2.c. */
        ...
        public static void Rfc6979HmacSha256Generate(Rfc6979HmacSha256T rng, byte[] outdata, int outlen)
        {
            ...
            while (outlen > 0)
            {
                ...
                Secp256K1.Util.Memcpy(rng.V, 0, outdata, index, now); //memcpy(outdata, rng.v, now);
                index += now;
                outlen -= now;
            }
            rng.Retry = true;
        ...
        {
            Secp256K1.Util.MemSet(rng.K, 0, 32); //memset(rng.k, 0, 32);
            Secp256K1.Util.MemSet(rng.V, 0, 32); //memset(rng.v, 0, 32);
            rng.Retry = false;
        }
    }
}
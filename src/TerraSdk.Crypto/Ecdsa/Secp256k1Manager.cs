...
using TerraSdk.Crypto.Ecdsa.Internal.Secp256K1;
    ...
    public class Secp256K1Manager
    {
        ...
        private static bool Secp256K1EcdsaSignRecoverable(Context ctx, EcdsaRecoverableSignature signature, byte[] msg32, byte[] seckey, NonceFunction noncefp, byte[] noncedata)
        {
            ...
            if (!overflow && !Scalar.IsZero(sec))
            {
                ...
                Internal.Secp256K1.Util.MemSet(nonce32, 0, 32); //memset(nonce32, 0, 32);
                Scalar.Clear(msg);
                Scalar.Clear(non);
                Scalar.Clear(sec);
            }
            ...
            {
                Internal.Secp256K1.Util.MemSet(signature.Data, 0, EcdsaRecoverableSignature.Size); //memset(signature, 0, sizeof(* signature));
            }
            return ret;
        }
